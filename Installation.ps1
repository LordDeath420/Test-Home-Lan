...
# Alte temporäre Logs bereinigen (älter als 1 Tag)
Remove-TemporaryLogs -LogDir $script:LogRoot
...
function Get-CallerTag {
    $helpers = @('Write-Log','Get-CallerTag','Write-HostLog','Write-LogLine','Write-ActLog')
...
function Export-InventoryReport {
    ...
    # CSVs (optional aber praktisch) - UTF8 mit BOM für Excel-Kompatibilität
    # PowerShell 5.1: -Encoding UTF8 schreibt mit BOM
    ...
    if ($net -and $net.Count -gt 0) {
        ...
        # Leere Netzwerk-CSV mit Header erstellen
        [pscustomobject]@{ Name=''; Status=''; LinkSpeed=''; MacAddress=''; IPv4='' } | 
            Export-Csv -Path $netCsv -NoTypeInformation -Encoding UTF8 -Force
    }

    Write-Log -Message "Inventar exportiert: $txtPath / $csvPath / $netCsv" -Level 'SUCCESS'
}

# Erstelle eine maschinenlesbare Zusammenfassung (JSON) für Automatisierung/Reporting
function Export-InstallationSummary {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)][string]$Name,
        [string]$OutDir = $script:LogRoot,
    ...
    $jsonPath = Join-Path $OutDir "$Name-summary.json"
    ...
    $summary = [ordered]@{
        ...
        Success         = $Success
        DurationMinutes = if ($Duration) { [math]::Round($Duration.TotalMinutes, 2) } else { $null }
        Errors          = $Errors
        System          = [ordered]@{
            OriginalName    = $env:COMPUTERNAME
            Manufacturer    = $cs.Manufacturer
            Model           = $cs.Model
            Mainboard       = $bb.Product
            MainboardSerial = $bb.SerialNumber
            OS              = $os.Caption
            Build           = $os.BuildNumber
            MacAddresses    = $macs
        }
        ScriptVersion   = "2.0.0"  # Versionsnummer für Tracking
    }
    
    try {
        $summary | ConvertTo-Json -Depth 4 | Out-File -FilePath $jsonPath -Encoding UTF8 -Force
        Write-Log -Message "Installations-Summary exportiert: $jsonPath" -Level SUCCESS
    } catch {
        Write-Log -Message "Konnte Summary nicht exportieren: $($_.Exception.Message)" -Level WARN
    }
}
...
function Write-Log {
    ...
    param(
        ...
        # Ziel-Log. Wenn nicht angegeben: $script:LOG (falls gesetzt) sonst $script:MainLogPath
        [string]$Path
    )

    if (-not $Path -or $Path.Trim() -eq '') {
        if ($script:LOG) { $Path = $script:LOG }
        elseif ($script:MainLogPath) { $Path = $script:MainLogPath }
        else { $Path = Join-Path $env:TEMP 'installation.log' }
    }

    # MainLog (für Spiegelung)
    if (-not $Tag -or $Tag.Trim() -eq '') { $Tag = Get-CallerTag }

    $main = $script:MainLogPath
    if (-not $main -or $main.Trim() -eq '') { $main = $Path }

    Initialize-LogFile -Path $Path
    if ($Path -ne $main) { Initialize-LogFile -Path $main }

    $ts   = (Get-Date).ToString('yyyy-MM-dd HH:mm:ss')
    $line = "[{0}] [{1}] [{2}] {3}" -f $ts, $Level, $Tag.ToUpper(), $Message

    # Konsole
    $colorMap = @{ 'INFO'='Gray'; 'WARN'='Yellow'; 'ERROR'='Red'; 'SUCCESS'='Green' }
    try { Write-Host $line -ForegroundColor ($colorMap[$Level]) } catch { Write-Host $line }

    # Datei (append, ohne Überschreiben)
    try { Add-Content -Encoding UTF8 -Path $Path -Value $line } catch {
        # Fallback mit Retry, falls Datei kurzzeitig gelockt ist
        $maxTry = 5
    ...
    # Spiegeln ins Hauptlog (nur wenn anderes Ziel)
    if ($Path -ne $main) {
        try { [System.IO.File]::AppendAllText($main, $line + [Environment]::NewLine, (New-Object System.Text.UTF8Encoding($true))) } catch {
            try { [System.IO.File]::AppendAllText($main, $line + [Environment]::NewLine, (New-Object System.Text.UTF8Encoding($true))) } catch {}
        }
    }
}
...
function Switch-ToNamedMainLog {
    ...
    $oldTranscript = $script:TranscriptPath
    ...
    if ($oldMain -and (Test-Path $oldMain) -and ($oldMain -ne $script:MainLogPath)) {
        ...
        # Alte temporäre DESKTOP-*.log löschen wenn Name mit DESKTOP- beginnt
        $oldBaseName = [System.IO.Path]::GetFileNameWithoutExtension($oldMain)
        if ($oldBaseName -match '^DESKTOP-') {
            try {
                Remove-Item -Path $oldMain -Force -ErrorAction SilentlyContinue
                Write-Log -Message "Temporäre Log-Datei gelöscht: $oldMain" -Level INFO
            } catch {}
            
            # Auch alte Transcript-Datei löschen falls vorhanden
            if ($oldTranscript -and (Test-Path $oldTranscript)) {
                try {
                    Remove-Item -Path $oldTranscript -Force -ErrorAction SilentlyContinue
                } catch {}
            }
        }
    }
...
# --- Encoding / Umlaute stabilisieren ---
try { chcp 65001 | Out-Null } catch {}
...
try {
    ...
    # Maximal m?gliche Fenstergr??e (physisch) auslesen
    $max = $raw.MaxPhysicalWindowSize
    if (-not $max.Width -or -not $max.Height) { $max = $raw.MaxWindowSize }

    # Buffer erst gro? genug machen, sonst Exception beim Vergr??ern des Fensters
...
"@
    ...
    Write-Warning "Konnte Fenster-/Puffergr??e nicht setzen: $_"
}
...
#region Konstanten
$Global:BasePath = "D:\Datein"
$Global:logRoot = Join-Path $BasePath "Macs_und_Mainboards_Logs"
$Global:BaseFilePath = Join-Path $BasePath "Installation\Dokumente"
$Global:LogPath  = Join-Path $BasePath "Macs_und_Mainboards_Logs"
$Global:FontsPath = Join-Path $BasePath "fonts"
$Global:TreiberPath = Join-Path $BasePath "treiber"
$Global:ProgramPath = Join-Path $BaseFilePath "Installation von Programmen"
$Global:LayoutXml = Join-Path $BaseFilePath "LayoutModification.xml"
$Global:WlanPwFile = Join-Path $BaseFilePath "wlan.txt"
$Global:MakFile = Join-Path $BaseFilePath "win10Ent-mak.txt"
$Global:TreiberMainboardPath = Join-Path $TreiberPath "Mainboard"
$Global:TreiberNotebookPath = Join-Path $TreiberPath "Notebook"
$Global:TreiberNucPath = Join-Path $TreiberPath "NUC"
$Global:doneDir = Join-Path $logRoot "erledigt"

# Validiere kritische Pfade beim Start
function Test-RequiredPaths {
    $criticalPaths = @{
        'Basis-Pfad'      = $Global:BasePath
        'Log-Verzeichnis' = $Global:LogPath
        'Treiber-Pfad'    = $Global:TreiberPath
    }
    
    $missingPaths = @()
    foreach ($entry in $criticalPaths.GetEnumerator()) {
        if (-not (Test-Path $entry.Value)) {
            $missingPaths += "  - $($entry.Key): $($entry.Value)"
        }
    }
    ...
    if ($missingPaths.Count -gt 0) {
        ...
        Write-Host "  WARNUNG: Kritische Pfade nicht gefunden!" -ForegroundColor Red
        Write-Host ("!" * 55) -ForegroundColor Red
        Write-Host ""
        ...
        Write-Host ""
        Write-Host "Bitte sicherstellen, dass das Installations-Laufwerk" -ForegroundColor Yellow
        Write-Host "angeschlossen ist (D:\Datein)." -ForegroundColor Yellow
        Write-Host ""
        
        $continue = Read-Host "Trotzdem fortfahren? (j/N)"
        if ($continue -notin @('j','ja','y','yes')) {
...
# Zeige aktuelle System-Informationen
function Show-SystemInfo {
    ...
    Write-Host "       AKTUELLE SYSTEM-INFORMATIONEN" -ForegroundColor DarkCyan
    Write-Host ("=" * 55) -ForegroundColor DarkCyan
    Write-Host ""
    Write-Host ("  Computername:   {0}" -f $env:COMPUTERNAME) -ForegroundColor Gray
    Write-Host ("  Hersteller:     {0}" -f $cs.Manufacturer) -ForegroundColor Gray
    Write-Host ("  Modell:         {0}" -f $cs.Model) -ForegroundColor Gray
    Write-Host ("  Mainboard:      {0}" -f $bb.Product) -ForegroundColor Gray
    Write-Host ("  OS:             {0}" -f $os.Caption) -ForegroundColor Gray
    Write-Host ("  Build:          {0}" -f $os.BuildNumber) -ForegroundColor Gray
    Write-Host ""
...
if ($paths.Count -eq 0) {
    ...
    # NEU: Auch DESKTOP-* und WIN-* (temporäre Windows-Namen) ausschließen
    $exclude = '(?i)-(Treiber|Fonts|transcript|userdelete|Einstellen|Network)$'
    $excludeTemp = '^(DESKTOP-|WIN-)'
    
    $allFiles = Get-ChildItem -Path $paths -File -ErrorAction SilentlyContinue |
        Where-Object { 
            $_.Extension -eq ".log" -and 
            $_.BaseName -ne 'Installation' -and
            $_.BaseName -notmatch $exclude -and
            $_.BaseName -notmatch $excludeTemp
        }

    if ($allFiles.Count -eq 0) {
        Write-HostLog "[INFO] Keine bisherigen Installationen gefunden." -ForegroundColor DarkYellow
    } else {
        ...
        Write-HostLog ""
        Write-HostLog ("=" * 50) -ForegroundColor Cyan
        Write-HostLog "  LETZTE 10 INSTALLIERTE RECHNER" -ForegroundColor Cyan
        Write-HostLog ("=" * 50) -ForegroundColor Cyan
        Write-HostLog ""
        Write-HostLog ("{0,-30} {1}" -f "RECHNERNAME", "DATUM") -ForegroundColor Yellow
        Write-HostLog ("-" * 50)
...
function Get-Manufacturer {
    ...
    try {
        $m1 = (Get-CimInstance Win32_ComputerSystem -ErrorAction Stop).Manufacturer
        if ($m1) { $src += " $m1" }
    } catch {}

    try {
        $m2 = (Get-CimInstance Win32_BaseBoard -ErrorAction Stop).Manufacturer
        if ($m2) { $src += " $m2" }
    } catch {}

    $src = $src.Trim().ToUpperInvariant()

    if ($src -match "LENOVO") { return "Lenovo" }
    if ($src -match "DELL")   { return "Dell" }
    if ($src -match "HEWLETT|HP INC|HP-") { return "HP" }
    if ($src -match "ASUSTeK|ASUS") { return "ASUS" }
    if ($src -match "ACER") { return "Acer" }
    if ($src -match "MSI") { return "MSI" }
    if ($src -match "GIGABYTE") { return "Gigabyte" }
...
function Drucker {
    ...
    if ($p) { Remove-Printer -Name $druckerName }
}

function Netzwerk {
    # Deaktiviert Energiesparen an Ethernet-Adapter (Registry)
    for ($id=0; $id -lt 255; $id++) {
        $cls = ("HKLM:\SYSTEM\CurrentControlSet\Control\Class\{0}\{1}" -f "{4D36E972-E325-11CE-BFC1-08002BE10318}",("{0:D4}" -f $id))
        $props = Get-ItemProperty -Path $cls -ErrorAction SilentlyContinue
        if ($null -eq $props) { continue }
        if (($props."*ifType" -eq 6) -and ($props.DeviceInstanceID -notlike "ROOT\*") -and ($props.DeviceInstanceID -notlike "SW\*")) {
            Set-ItemProperty -Path $cls -Name "PnPCapabilities" -Value 24 -Type DWord -ErrorAction SilentlyContinue
        }
    }
}
...
function Install-UltraVNCServer {
    param(
        [string]$Password  = 'qakaqq',  # Default-Passwort als String
        [switch]$ViewOnly,
...
function Setup-BaseSystem {
	...
	$job = Start-Job -ScriptBlock {
		param($file)
		Write-HostLog "?? Live-Log gestartet für $file ..." -ForegroundColor Yellow
    ...
    net user Administrator @aka@ | Out-Null
...
function Setup-NUC {
    ...
    Write-HostLog "?? Live-Log l?uft direkt in der Konsole ..." -ForegroundColor Yellow
...
function Setup-VPN {
    ...
    try {
        ...
        if (-not (Test-Path $path)) {
            Write-HostLog "??  Registry-Pfad fehlt, wird angelegt: $path" -ForegroundColor Yellow
        ...
        if (-not (Test-Path $appPriv)) {
            Write-HostLog "??  Lege Registry-Pfad an: $appPriv" -ForegroundColor Yellow
        ...
        Write-Warning "?? Fehler beim Setzen der Policies: $($_.Exception.Message)"
    }
    ...
    try {
        ...
        # ----------------------------------------------------------
        if (Test-Path $avkReg) {
            Write-HostLog "??  Importiere AVKCLIENT.reg..." -ForegroundColor Yellow
        ...
        # ----------------------------------------------------------
        if (Test-Path $agent) {
            Write-HostLog "??  Installiere G DATA Agent (__windowsfullagent.exe)..." -ForegroundColor Yellow
            ...
            if ($process.ExitCode -eq 0) {
                ...
                Write-Warning "?? G DATA Agent-Installer meldete ExitCode $($process.ExitCode). Siehe Log: $logFileGD"
            }
        } else {
        ...
        Write-Warning "?? Fehler beim Installieren von AVK/G DATA: $($_.Exception.Message)"
    }
...
"@ | Out-File (Join-Path $Global:LogPath "$Name.txt")
	...
	# ==========================================================
	try {
		Write-HostLog "??  Setze OOBE- und Erstlogin-Parameter korrekt..." -ForegroundColor Yellow
		...
		Write-Warning "?? Konnte OOBE-Fix nicht anwenden: $($_.Exception.Message)"
	}
...
# Funktion: Rechnernamen validieren
function Test-ValidComputerName {
    param([string]$Name)
    
    if ([string]::IsNullOrWhiteSpace($Name)) { return $false }
    if ($Name.Length -gt 15) { return $false }  # NetBIOS-Limit
    if ($Name.Length -lt 1) { return $false }
    
    # Nur erlaubte Zeichen: A-Z, a-z, 0-9, Bindestrich (nicht am Anfang/Ende)
    ...
    if ($Name.ToUpper() -in $reserved) { return $false }
...
function Show-InstallMenu {
    ...
    Write-Host "       INSTALLATIONS-TOOL - HAUPTMENÜ" -ForegroundColor Cyan
    Write-Host ("=" * 55) -ForegroundColor Cyan
    Write-Host ""
    Write-Host "  [1] Rechner          - Standard-PC Installation" -ForegroundColor White
    Write-Host "  [2] Baaske           - Baaske-Gerät" -ForegroundColor White
    Write-Host "  [3] ImportRechner    - Import-PC mit VNC" -ForegroundColor White
    Write-Host "  [4] NUC              - Intel NUC" -ForegroundColor White
    Write-Host "  [5] Laptop           - Notebook" -ForegroundColor White
    Write-Host "  [6] VPN              - VPN-Gerät" -ForegroundColor White
    Write-Host ""
    Write-Host "  [7] Einstellen       - Nur Einstellungen" -ForegroundColor DarkGray
    Write-Host "  [8] TreiberEntpacken - Treiber extrahieren" -ForegroundColor DarkGray
    Write-Host "  [9] WindowsUpdateReset - WU zurücksetzen" -ForegroundColor DarkGray
    Write-Host ""
    Write-Host ("-" * 55) -ForegroundColor DarkGray
}
...
Write-Host ""
Write-HostLog "Ausgewählt: $modus" -ForegroundColor Green
Write-Host ""

# Rechnername mit Validierung
do {
    $Name = Read-Host "Neuer Rechnername (max. 15 Zeichen)"
    $Name = $Name.Trim()
    
    if (-not $Name) {
        Write-HostLog "Rechnername darf nicht leer sein." -ForegroundColor Yellow
    }
    elseif (-not (Test-ValidComputerName -Name $Name)) {
        Write-HostLog "Ungültiger Rechnername. Regeln:" -ForegroundColor Yellow
        Write-HostLog "  - Max. 15 Zeichen" -ForegroundColor DarkYellow
        Write-HostLog "  - Nur Buchstaben, Zahlen, Bindestriche" -ForegroundColor DarkYellow
        Write-HostLog "  - Kein Bindestrich am Anfang/Ende" -ForegroundColor DarkYellow
        $Name = $null
    }
} until ($Name)

# --- Logging: ab hier auf $Name.log umschalten (inkl. Transcript) ---
try { Switch-ToNamedMainLog -Name $Name } catch {}

# HardwareID und TicketID mit optionaler Eingabe
$HWId = Read-Host "HardwareID eingeben (optional, Enter zum Überspringen)"
$TID  = Read-Host "TicketID eingeben (optional, Enter zum Überspringen)"
...
# Modell-Eingabe für bestimmte Modi
if ($modus -in @('Laptop','VPN','NUC')) {
    Write-Host ""
    $beispiele = switch ($modus) {
        'NUC'    { "z.B. UN62, NUC11TNHi3, NUC12" }
        'Laptop' { "z.B. V15 IIL, V15 G4 AMN, ThinkPad T14" }
        'VPN'    { "z.B. V15 IIL, V15 G4 AMN" }
    }
    do {
        $Modell = Read-Host "Modell-Reihe ($beispiele)"
        if (-not $Modell) { Write-HostLog "Modell darf nicht leer sein." -ForegroundColor Yellow }
    } until ($Modell)
}

# Standort für Laptop/NUC
if ($modus -in @('Laptop','NUC')) {
    Write-Host ""
    Write-Host "Verfügbare Standorte:" -ForegroundColor Cyan
    Write-Host "  [1] Bochum" -ForegroundColor White
    Write-Host "  [2] Hattingen" -ForegroundColor White
    Write-Host "  [3] Linden" -ForegroundColor White
    Write-Host "  [4] Extern" -ForegroundColor White
    
    do {
        $standortAuswahl = Read-Host "Standort (1-4)"
        $Standort = switch ($standortAuswahl) {
            '1' { 'Bochum' }
            '2' { 'Hattingen' }
            '3' { 'Linden' }
            '4' { 'Extern' }
        ...
        if (-not $Standort) { Write-HostLog "Bitte 1-4 eingeben." -ForegroundColor Yellow }
    } until ($Standort)
    
    Write-HostLog "Standort: $Standort" -ForegroundColor Green
}
...
Write-Host ("=" * 55) -ForegroundColor Cyan
Write-Host ""
Write-Host ("  Modus:        {0}" -f $modus) -ForegroundColor White
Write-Host ("  Rechnername:  {0}" -f $Name) -ForegroundColor White
if ($HWId) { Write-Host ("  HardwareID:   {0}" -f $HWId) -ForegroundColor White }
if ($TID)  { Write-Host ("  TicketID:     {0}" -f $TID) -ForegroundColor White }
if ($Modell) { Write-Host ("  Modell:       {0}" -f $Modell) -ForegroundColor White }
if ($Standort) { Write-Host ("  Standort:     {0}" -f $Standort) -ForegroundColor White }
Write-Host ""
Write-Host ("-" * 55) -ForegroundColor DarkGray

if (-not (Ask-YesNo "Installation starten?" -DefaultYes)) {
    Write-HostLog "Installation abgebrochen." -ForegroundColor Yellow
    exit 0
}
...
# Führe Installation mit Error-Handling aus
try {
    switch ($modus) {
        ...
        "ImportRechner" { Setup-BaseSystem -Name $Name -HWId $HWId -TID $TID -MitTreibern:$false; Install-UltraVNCServer -Password 'qakaqq' }
    ...
    $script:InstallErrors += $_.Exception.Message
    Write-Log -Message "KRITISCHER FEHLER: $($_.Exception.Message)" -Level ERROR
    Write-HostLog "FEHLER: $($_.Exception.Message)" -ForegroundColor Red
}

$script:InstallEndTime = Get-Date
$duration = $script:InstallEndTime - $script:InstallStartTime

Write-Progress -Activity "Installation $modus" -Completed

# Abschlussmeldung
Write-Host ""
Write-Host ("=" * 55) -ForegroundColor $(if ($installSuccess) { 'Green' } else { 'Red' })
if ($installSuccess) {
...
Write-Host ""
Write-Host ("  Rechnername:  {0}" -f $Name) -ForegroundColor White
Write-Host ("  Dauer:        {0:mm}:{0:ss} Minuten" -f $duration) -ForegroundColor White
Write-Host ("  Log-Datei:    {0}" -f $script:MainLogPath) -ForegroundColor DarkGray
Write-Host ""
...
# Exportiere maschinenlesbare Zusammenfassung
try {
    Export-InstallationSummary -Name $Name -Modus $modus -TicketID $TID -HardwareID $HWId `
        -Success $installSuccess -Errors $script:InstallErrors -Duration $duration
} catch {
    Write-Log -Message "Summary-Export fehlgeschlagen: $($_.Exception.Message)" -Level WARN
}
